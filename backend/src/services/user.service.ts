...
export const createUserService = async (
  ...
  try {
    ...
    if (
      ...
      (error.message && error.message.toLowerCase().includes("duplicate entry"))
    ) {
...
export const loginUserService = async (
  ...
  try {
    const userWithPasswordHash =
      await userModel.findUserByEmailWithPasswordFromDB(email);
...
export const updateUserPasswordService = async (
  ...
  try {
    ...
    await userModel.updateUserPasswordInDB(userId, hashedPassword);
  } catch (error: any) {
    console.error("Update user password service error:", error.message);
