...
export const createUserInDB = async (
  ...
  try {
    ...
    if (!createdUser) {
      throw new Error(
        "Failed to retrieve created user from DB after insertion."
      );
    }
    return createdUser;
  } catch (dbError: any) {
...
export const findUserByEmailWithPasswordFromDB = async (
  email: string
...
export const updateUserPasswordInDB = async (
  ...
  const [result] = await query<ResultSetHeader>(sql, [hashedPassword, userId]);
